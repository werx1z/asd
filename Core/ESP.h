...
struct Vector2 { float x, y; bool visible; Vector2() : x(0), y(0), visible(false) {} Vector2(float x, float y) : x(x), y(y), visible(true) {} };

struct Player {
    uintptr_t address;
    Vector3 position, head_position;
    Vector2 screen_pos, screen_head;
    int health, armor, team;
    bool dead;
    char name[32];
    float distance;
    bool visible;
...
private:
    ...
    static void draw_box(Vector2 top, Vector2 bottom, int color);
...
public:
    ...
    static int get_local_team() { return local_team; }
};